...
class ConversationsService:
    ...
    def create_conversation(self, payload: ConversationCreate) -> Chat:
        ...
        configuration = payload.configuration.model_dump_json(exclude={"model"})
        user = get_object_or_404(User, id=UUID(payload.user_id))
        model = get_object_or_404(AIModel, slug_name=slug_model)
        chat = Chat.objects.create(model=model, configuration=configuration)
        chat.users.add(user)
        return chat
    ...
    def list_configuration_options(self) -> ChatConfigurationOptions:
        ...
        return ChatConfigurationOptions(models=models, temperatures=temperatures)
